...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        'relative flex flex-col h-full transition-all duration-300 ease-in-out bg-card border-r z-20',
        collapsed ? 'w-16' : 'w-64'
      )}
    >
      ...
      <div className="flex items-center gap-3 px-4 py-5 border-b">
        <div className="shrink-0 w-8 h-8 rounded-lg bg-primary flex items-center justify-center text-primary-foreground font-bold text-sm shadow-sm">
          ⚡
        </div>
        {!collapsed && (
          <div className="min-w-0">
            <p className="text-foreground font-bold text-sm leading-tight">E-Sport</p>
            <p className="text-muted-foreground text-xs font-medium">Player Ranking</p>
          </div>
        )}
      </div>
      ...
      <button
        ...
        className="absolute -right-3 top-24 w-6 h-6 rounded-full bg-muted border flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors duration-200 z-10"
      >
      ...
      {!collapsed && (
        <div className="px-4 py-3 border-t">
          <p className="text-muted-foreground text-xs">v1.0 • Mock Data</p>
        </div>
      )}
    </aside>
  );
